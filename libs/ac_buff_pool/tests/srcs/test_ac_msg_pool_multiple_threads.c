...
static ac_bool mptt_process_msg(AcComp* this, AcMsg* msg) {
  ...
  if (params->extra_size != 0) {
    for (ac_u32 i = 0; i < params->extra_size; i++) {
      mt->extra_error |= msg->extra_data[i] != params->extra_value;
      if (mt->extra_error) {
        mt->extra_error_index = i;
        mt->extra_error_value = msg->extra_data[i];
        mt->extra_error_expected_value = params->extra_value;
        ac_debug_printf("mptt_process_msg: err msg->arg1=%ld msg->arg2=%ld count=%ld "
...
ac_bool test_msg_pool_multiple_threads(ac_u32 thread_count, ac_u32 comps_per_thread,
  ...
  ac_printf("test_msg_pool_multiple_threads:+ thread_count=%d comps_per_thread=%d extra_size=%d rl=%d\n",
  ...
  AcMsgPool* mp = AcMsgPool_create_extra(msg_count, extra_size);
  ...
  for (ac_u32 message = 0; !error && (message < MSGS_PER_THREAD); message++) {
    ...
    for (ac_s32 i = 0; !error && (i < thread_count); i++) {
      ...
      if (msg != AC_NULL) {
        ...
        msg->arg2 = ac_tscrd();
        if (extra_size != 0) {
          ac_memset(msg->extra_data, params[i]->extra_value, params[i]->extra_size);
        }
  ...
  for (ac_u32 thrd = 0; thrd < thread_count; thrd++) {
    ...
    for (; count-- > 0; idx = (idx + 1) % max) {
      ...
      error |= AC_TEST(msg_tsc->extra_error == AC_FALSE);
