...
int main(void) {
  ...
  ac_thread_init(3);
  //AcReceptor_init(256); // Corrupted double-link list error when AC_PLATFORM == Posix
  //AcReceptor_init(440); // Futex error when AC_PLATFORM == Posix
  AcReceptor_init(441); // OK
  AcTime_init();
