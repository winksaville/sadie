 ...
 * This is a shortened walking 1 bit test.
 */
static struct test_case_pdpte_1g_fields {
  ...
  ac_u64 pke;
  ac_u64 xd;
} test_case_pdpte_1g_fields_array[] = {
  { .val.raw=0x1, .p=0x1, },
  { .val.raw=0x2, .rw=0x1, },
  { .val.raw=0x4, .us=0x1, },
  { .val.raw=0x8, .pwt=0x1, },
  ...
  { .val.raw=0x400000000000000, .phy_addr=0x10000000, },
  { .val.raw=0x800000000000000, .pke=0x1, },
  { .val.raw=0x4000000000000000, .pke=0x8, },
  { .val.raw=0x8000000000000000, .xd=0x1, },
};

static ac_bool test_pdpte_1g_fields(struct test_case_pdpte_1g_fields* test) {
  ac_bool error = AC_FALSE;

  print_pdpte_1g_fields("", test->val.raw);
 ...
 * This is a shortened walking 1 bit test.
 */
static struct test_case_pdpte_fields {
  union pdpte_fields_u val;
  ac_u64 p;
  ac_u64 rw;
  ac_u64 us;
...
static ac_bool test_pdpte_fields(struct test_case_pdpte_fields* test) {
  ac_bool error = AC_FALSE;
 ...
 * This is a shortened walking 1 bit test.
 */
static struct test_case_pde_2m_fields {
  union pdpte_fields_u val;
  ac_u64 p;
  ac_u64 rw;
  ac_u64 us;
...
static ac_bool test_pde_2m_fields(struct test_case_pde_2m_fields* test) {
  ac_bool error = AC_FALSE;

  print_pde_2m_fields("", test->val.raw);
...
static ac_bool test_pde_2m_fields_array(void) {
  ac_bool error = AC_FALSE;

  for (ac_uint i = 0;
      i < AC_ARRAY_COUNT(test_case_pde_2m_fields_array); i++) {
    error |= test_pde_2m_fields(&test_case_pde_2m_fields_array[i]);
  }
